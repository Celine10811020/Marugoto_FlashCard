const state = {
  ...
  flipped: false,        // 是否翻面
  backField: null        // 背面顯示欄位："HanZi" 或 "Chinese"
};
...
function setCard(jp, backText, backField){
  ...
  el.backLabel.textContent = backField === 'Chinese' ? '中文（背面）' : backField === 'HanZi' ? '漢字（背面）' : '—';
}
...
function showBack(field){
  ...
  state.flipped = true;
  setCard(row.Japanese, row[field] ?? '', field);
...
async function loadLevel(level){
  try{
    ...
    rows = rows.map(r=>{
      ...
      return { Lesson: o.Lesson, Japanese: o.Japanese, HanZi: o.HanZi, Chinese: o.Chinese };
    }).filter(r=> r.Japanese || r.HanZi || r.Chinese);
